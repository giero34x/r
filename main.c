...
const uint32_t VeloMin = 10;
const uint32_t VeloMax = 100;
const uint32_t AccInc = 2;
const uint32_t DccInc = 2;

bool enablePWM = false;
bool enablePWM_LED = true;
...
int main(void)
{
    ...
    uint32_t pinRed = 1;  	//dioda czerwona 
    uint32_t pinWh = 2;		//dioda biala
    uint32_t pinMot = 3;	//pwm silnika
    ...
    app_pwm_config_t pwm1_cfg = 
      APP_PWM_DEFAULT_CONFIG_2CH(5000L, pinRed, pinWh);
    ...
    app_pwm_config_t pwm2_cfg = 
      APP_PWM_DEFAULT_CONFIG_1CH(5000L, pinMot);
    ...
    // main loop:
    bool pwmEnabledVelo = true;
	bool pwmEnabledLED = false;

	
    while(1) {
		
        // enable disable as needed for motor control
        if(!enablePWM) {
			
          if(pwmEnabledVelo) {
			  
				//disablePWM if enabled
            app_pwm_disable(&PWM2);
            nrf_drv_gpiote_out_task_disable(pinMot);
            nrf_gpio_cfg_output(pinMot);
            nrf_gpio_pin_clear(pinMot);
          

            pwmEnabledVelo = false; //info flag
          }
        }
          ...
          if(!pwmEnabledVelo) {
            ...
            nrf_drv_gpiote_out_task_enable(pinMot);
            app_pwm_enable(&PWM2);
			
			
            pwmEnabledVelo = true; //info flag
          }
        }

        if(pwmEnabledVelo) {
          // Set the duty cycle - keep trying until PWM is ready
          while (app_pwm_channel_duty_set(&PWM2, 0, valVelo) == NRF_ERROR_BUSY);
        }
        
		///////////////////////////////////////////////////////
		//////dac TUTAJ zmiane wartoci pwm
		///////////////////////////////////////////////////////
      ...
      nrf_delay_ms(delay);
		...
		//////////////////////////////////////////////////////////////////////////////////////	
// przenies na przerwanie ? ? ?	
// obczaic lub zmienic na BLE_ROLLY\sdk's\SDK\nRF5_SDK_14.2.0_17b948a\nRF5_SDK_14.2.0_17b948a\examples\peripheral\led_softblink
